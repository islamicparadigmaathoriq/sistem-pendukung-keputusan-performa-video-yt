...
class UserInterface:
    ...
    def render_category_comparison(self, main_cat, comp_cats):
        ...
        # Card Channel Utama
        with cols[0]:
            st.markdown(f"<div style='background-color:{main_cat['color']}10; border:2px solid {main_cat['color']}; padding:20px; border-radius:15px; height:100%;'><h4 style='color:{main_cat['color']}; text-align:center;'>📺 CHANNEL UTAMA</h4><div style='text-align:center; margin:15px 0;'><h2 style='color:{main_cat['color']};'>{main_cat['category']}</h2><p style='font-size:24px; font-weight:bold;'>{main_cat['subs']:,}</p><p style='color:#666;'>subscribers</p></div><hr><p><strong>🎯 Target:</strong> {main_cat['benchmark']['subs_target']}</p><p><strong>🔍 Focus:</strong> {main_cat['benchmark']['focus']}</p></div>", unsafe_allow_html=True)
        
        # Cards Kompetitor
        for i, comp_cat in enumerate(comp_cats):
            with cols[i + 1]:
                st.markdown(f"<div style='background-color:{comp_cat['color']}10; border:2px solid {comp_cat['color']}; padding:20px; border-radius:15px; height:100%;'><h4 style='color:{comp_cat['color']}; text-align:center;'>🎭 KOMPETITOR {i+1}</h4><div style='text-align:center; margin:15px 0;'><h2 style='color:{comp_cat['color']};'>{comp_cat['category']}</h2><p style='font-size:24px; font-weight:bold;'>{comp_cat['subs']:,}</p><p style='color:#666;'>subscribers</p></div><hr><p><strong>🎯 Target:</strong> {comp_cat['benchmark']['subs_target']}</p><p><strong>🔍 Focus:</strong> {comp_cat['benchmark']['focus']}</p></div>", unsafe_allow_html=True)
        ...
        if comp_cats:
            ...
            if all(cl == main_level for cl in comp_levels):
                insights.append({
                    ...
                    'desc': f"Semua channel berada di level **{main_cat['category']}**. Fokus pada diferensiasi konten dan engagement untuk unggul."
                })
                ...
                if higher:
                    insights.append({
                        ...
                        'desc': f"Ada kompetitor di level lebih tinggi. **Pelajari strategi mereka**: jenis konten, frekuensi upload, engagement tactics, dan kolaborasi."
                    })
                ...
                if lower:
                    insights.append({
                        ...
                        'desc': f"Anda lebih besar dari beberapa kompetitor. Manfaatkan untuk: authority positioning, mentorship content, dan community building."
                    })
